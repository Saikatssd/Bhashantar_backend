...
exports.logRevertAction = async (req, res, next) => {
    ...
    try {
        ...
        if (!projectId || !fileId || !userId || !reason) {
            return next(new ErrorHandler("Missing required fields: projectId, fileId, userId, reason", 400));
        }
        ...
        res.status(201).send({ id: docRef.id, ...revertDoc });
    } catch (error) {
...
exports.recordFileSubmission = async (req, res, next) => {
    const { projectId, documentId, userId, userName, fileName, fileUrl, companyId } = req.body;

    try {
        // Validate required fields
        if (!projectId || !documentId || !userId || !userName || !fileUrl || !fileName || !companyId) {
            return next(new ErrorHandler("Missing required fields: projectId, documentId, userId, userName,fileUrl, fileName, companyId", 400));
        }
...
exports.fetchSubmissionHistory = async (req, res, next) => {
    ...
    try {
        ...
        if (!projectId || !documentId) {
            return next(new ErrorHandler("Missing required fields: projectId, documentId", 400));
        }
        ...
        const submissionHistory = submissionSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
            submissionDate: doc.data().submissionDate ? doc.data().submissionDate : null,
        }));
